...
import { generateDaoSpecFile } from './dao/generate-dao-spec-file';

export async function generateInfraPersistence(props: Props) {
  // Cria a pasta infra/driven/persistence.
  const infraPersistencePath = path.join(props.modulePath, 'infra', 'driven', 'persistence');
  ...
  await fs.writeFile(
    path.join(infraPersistencePath, `${props.moduleName}.dao.spec.ts`),
    generateDaoSpecFile(props)
  );
}
